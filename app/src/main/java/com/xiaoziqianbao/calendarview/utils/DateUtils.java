package com.xiaoziqianbao.calendarview.utils;
...
public class DateUtils {
     ...
     * @return
     */
    public static List<Integer> getDayOfMonth(int year, int month) {
        ...
        for (int i = 0; i < days.length; i++) {
            for (int j = 0; j < days[i].length; j++) {
                if (i == 0 && j < daysOfFirstWeek - 1) {
                    arrayList.add(daysOfLastMonth - daysOfFirstWeek + 2 + j) ;
                } else if (dayNum <= daysOfMonth) {
                  arrayList.add(dayNum++) ;
                } else {
                    arrayList.add(nextDayNum++) ;
                }
            }
        }
        return arrayList;
